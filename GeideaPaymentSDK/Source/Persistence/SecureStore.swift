...
public struct SecureStore {
    ...
    private func error(from status: OSStatus) -> SecureStoreError {
        if #available(iOS 11.3, *) {
            _ = SecCopyErrorMessageString(status, nil) as String? ?? NSLocalizedString("Unhandled Error", comment: "")
        } else {
            // Fallback on earlier versions
        }
        return SecureStoreError.unhandledError(message: "Unhandled Error")
    }
}
